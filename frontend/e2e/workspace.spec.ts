...
test("operator workflow works without browser errors", async ({ page, request }, testInfo) => {
  ...
  const merchant = await request.post("http://127.0.0.1:8000/merchants", { headers, data: { merchant_id: merchantId, name: `E2E ${suffix} Store`, vertical: "ecommerce", payment_provider: "razorpay", razorpay_account_id: `acc_e2e_${suffix}`, freshdesk_domain: "", average_order_value: 2500, chargeback_history_count: 0, transaction_volume_30d_by_network: { VISA: 1000 } } });
  expect([201, 409]).toContain(merchant.status());
  const chargeback = await request.post("http://127.0.0.1:8000/webhook/chargeback", { headers, data: { chargeback_id: chargebackId, reason_code: "10.4", card_network: "VISA", dispute_amount: 1499, currency: "INR", filing_deadline: new Date(Date.now() + 72 * 60 * 60 * 1000).toISOString(), merchant_id: merchantId, order_id: `order_${suffix}`, payment_id: `pay_${suffix}` } });
  ...
  const simulatorMerchant = page.locator('select[name="merchant_id"]');
  ...
  const [simulationResponse] = await Promise.all([
    page.waitForResponse(response => response.url().includes("/dev/razorpay-simulator/disputes") && response.request().method() === "POST"),
    page.getByRole("button", { name: "Create test dispute" }).click(),
  ]);
  expect(simulationResponse.status(), await simulationResponse.text()).toBe(200);
  await expect(page.getByText(/Test dispute .* was created and delivered locally/)).toBeVisible({ timeout: 20_000 });
