...
def _packet() -> dict:
    return {
        ...
        "evidence_status": {
            ...
            "order_timeline": False,
        },
        "strongest_evidence": ["3DS authentication completed"],
    }


def test_quality_check_approves_valid_rebuttal_packet(tmp_path) -> None:
...
def test_quality_check_rejects_invalid_json(tmp_path) -> None:
    path = tmp_path / "rebuttal.json"
    ...
    assert result["quality_approved"] is False
    assert result["quality_rejection_reason"] == "Rebuttal document is not valid JSON."
